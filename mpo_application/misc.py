...
def check_randomized_apply(H, psi, cap, output, j, verbose=False, cutoff=1e-10):
    ...
    if verbose:
        print(
            "Randomized error check {:6.2e} ({:6.2e}, {:6.2e})".format(
                np.abs(result - result2) / np.abs(result), result, result2
            )
        )

    if abs(result - result2) > cutoff * abs(result):
        raise UserWarning(
            "Random check indicates descrepancy between answer and algorithm output ({}, {})".format(
                result, result2
            )
        )
