...
def round_left(mps, **kwargs):
    ...
    for i in range(mps.N - 2, 0, -1):
        ...
        U, s, Vt = truncated_svd(
            np.reshape(A, (A.shape[0], A.shape[1] * A.shape[2])), **kwargs
        )
...
def density_matrix_rounding(mps, stop=Cutoff(1e-14)):
    ...
    for i in range(1, N - 1):
        ...
        E[i] = E[i].reshape(
            E[i].shape[0], mps[i].shape[1], mps[i].shape[2]
        )  # Reshape to (A,d,Y)

        # E[i] = np.einsum("AdY,AdB->YB",E[i],mps_c[i])
        E_transposed = E[i].transpose(2, 0, 1)
        E[i] = E_transposed.reshape(E_transposed.shape[0], -1) @ mps_c[i].reshape(
            -1, mps_c[i].shape[-1]
        )
    ...
    for j in reversed(range(1, N - 1)):
        ...
        rho = rho_transposed.reshape(-1, rho_transposed.shape[2]) @ bottom.reshape(
            bottom.shape[0], -1
        )  # Resulting shape dk,lj
        ...
        M_top = U.reshape(U.shape[0], -1) @ top_transposed.reshape(
            -1, top_transposed.shape[-1]
        )
        M_top = M_top.T
...
def dass_round(mps, stop=Cutoff(1e-14)):
    ...
    for i in range(1, mps.N - 1):
        ...
        Q, R = la.qr(
            np.reshape(A, (A.shape[0] * A.shape[1], A.shape[2])), mode="reduced"
        )
        U, s, Vt = truncated_svd(R, stop=stop)
...
def orth_then_rand(mps, r):
    ...
    for i in range(1, mps.N - 1):
        ...
        Q, _ = la.qr(
            np.reshape(sketch, (sketch.shape[0] * sketch.shape[1], sketch.shape[2])),
            mode="reduced",
        )
...
def nystrom_round(mps, l, stop=no_truncation()):
    ...
    for j in range(1, mps.N - 1):
        ...
        mps[j] = np.reshape(
            mps[j], (right_factor.shape[0], mps[j].shape[1], left_factor.shape[1])
        )
