...
class IncrementalQR(object):
    ...
    def append(self, new_data):
        ...
        if self._libincrementalqr_available():
            ...
            self.data[:, self.n : self.n + k] = ormqr(
                "L",
                transpose,
                self.data[:, : self.n],
                self.tau[: self.n],
            ...
            geqrf = sp.linalg.get_lapack_funcs("geqrf", dtype=self.data.dtype)
            (
                self.data[self.n :, self.n : self.n + k],
                self.tau[self.n : self.n + k],
                _,
                _,
            ) = geqrf(self.data[self.n :, self.n : self.n + k])

            # Invert triangular part of new QR factor
            trtri = sp.linalg.get_lapack_funcs("trtri", dtype=self.data.dtype)
            self.data[self.n : self.n + k, self.n : self.n + k] = trtri(
                self.data[self.n : self.n + k, self.n : self.n + k]
            )[0]

            # Set top left part of new data
            self.data[: self.n, self.n : self.n + k] = (
                -np.triu(self.data[: self.n, : self.n])
                @ self.data[: self.n, self.n : self.n + k]
                @ np.triu(self.data[self.n : self.n + k, self.n : self.n + k])
            )
    ...
    def error_estimate(self):
        ...
        if self._libincrementalqr_available():
            ...
            return np.sqrt(
                sum(
                    np.linalg.norm(np.triu(self.data[: self.n, : self.n]), axis=1) ** -2
                )
                / self.n
            )
...
if __name__ == "__main__":
    ...
    print(
        "Total time for C++ implementation (complex)\t{}".format(total_time_cpp_complex)
    )
    ...
    print(
        "Total time for Scipy implementation (complex)\t{}".format(
            total_time_sp_complex
        )
    )
