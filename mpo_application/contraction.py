...
__all__ = ["mps_mpo_blas", "random_contraction", "random_contraction_inc", "density_matrix", "zipup", "fit"]
...
def mps_mpo_blas(
    ...
    mpo_reshaped = mpo[0].reshape(
        mpo[0].shape[0] * mpo[0].shape[1], mpo[0].shape[2]
    )  # Reshape to (dD,l)
    site = mpo_reshaped @ mps[0]  # Resulting shape: (dD,X)
    site = site.reshape(
        mpo[0].shape[0], mpo[0].shape[1] * mps[0].shape[1]
    )  # Reshape to (d,DX)
    new_mps.append(site)
    ...
    for i in range(1, mps.N - 1):
        ...
        mps_reshaped = mps_transposed.reshape(
            mps_transposed.shape[0], -1
        )  # Reshape to (l,XY)
        site = mpo_reshaped @ mps_reshaped  # Resulting shape (DdE,XY)
        site = site.reshape(
            mpo[i].shape[0],
            mpo[i].shape[1],  # Reshape to (D,d,E,X,Y)
            mpo[i].shape[2],
            mps[i].shape[0],
            mps[i].shape[2],
        )
        ...
        site = site.reshape(
            site.shape[0] * site.shape[1], site.shape[2], site.shape[3] * site.shape[4]
        )  # Reshape to (DE,d,XY)
        new_mps.append(site)

    # site = np.tensordot(mpo[-1], mps[-1], (2, 1))
    # site = np.einsum("Ddl,Xl->XDd",mpo[-1],mps[-1])
    mpo_reshaped = mpo[-1].reshape(
        mpo[-1].shape[0] * mpo[-1].shape[1], -1
    )  # Reshape to (Dd,l)
    site = mpo_reshaped @ mps[-1].T  # Resulting shape (Dd,X)
    site = site.reshape(
        mpo[-1].shape[0], mpo[-1].shape[1], mps[-1].shape[0]
    )  # Reshape to (D,d,X)
    site = site.transpose(0, 2, 1)  # Transpose to (D,X,d)
    site = site.reshape(
        site.shape[0] * site.shape[1], site.shape[2]
    )  # Reshape to (DX,d)
    new_mps.append(site)
    ...
    if stop.is_truncation():
        if round_type == "standard":
            ...
            round_type == "rand_then_orth_blas"
        ):  # TODO: Strange runtime behavior for r=None
...
def random_contraction(
    ...
    if n == 1:
        raise NotImplementedError(
            "MPO-MPS product for n=1 has not been implemented yet"
        )
        # return [H[0] * psi[0]]
    ...
    reshaped_psis = [
        psi[k]
        .transpose(1, 0, 2)
        .reshape(psi[k].shape[0] * psi[k].shape[1], psi[k].shape[2])
        for k in range(1, n - 1)
    ]
    reshaped_H = [
        H[k].transpose(0, 2, 3, 1).reshape(-1, H[k].shape[1]) for k in range(1, n - 1)
    ]

    reshaped_psis2 = [psi[j].reshape(psi[j].shape[0], -1) for j in range(1, n - 1)]
    reshaped_H2 = [
        H[j].transpose(1, 2, 0, 3).reshape(-1, H[j].shape[0] * H[j].shape[3])
    ...
    for j in reversed(range(1, n)):
        ...
        if j == n - 1:
            ...
            sketch = np.zeros(
                (visible_dim, cap_dim, current_sketchdim), dtype="complex"
            )
        ...
        while True:
            ...
            for idx in range(len(envs), current_sketchdim):
                ...
                for k in range(1, j):
                    ...
                    temp = result.reshape(
                        temp.shape[1], temp.shape[2], env[k - 1].shape[1]
                    )
            ...
            for idx in range(sketches_complete, current_sketchdim):
                if j == n - 1:
                    ...
                    temp = (H[j].reshape(H[j].shape[1], -1) @ temp.reshape(-1)).reshape(
                        H[j].shape[1]
                    )
            ...
            if not (outputdim is None):
                ...
                if current_sketchdim == current_maxdim:
                    ...
                    err_est = np.sqrt(
                        sum(np.linalg.norm(G, axis=0) ** (-2)) / current_sketchdim
                    )
                    done = (
                        err_est <= cutoff * norm_est
                        and current_sketchdim >= current_mindim
                    )
            ...
            if done:
                ...
                if j == n - 1:
                    ...
                    temp = np.conj(Q).T @ H[j].transpose(1, 0, 2).reshape(
                        H[j].shape[1], -1
                    )
                    ...
                    cap = np.dot(temp_, psi_j_.T).reshape(
                        temp.shape[0], temp.shape[1], psi[j].shape[0]
                    )
                else:
                    ...
                    temp = temp.reshape(
                        Q.shape[0], Q.shape[2], cap.shape[1], cap.shape[2]
                    )

                    # temp = np.einsum("ijkl,mikn->jlmn",temp, H[j])
                    temp_transposed = temp.transpose(1, 3, 0, 2)
                    Hj_transposed = H[j].transpose(1, 2, 0, 3)
                    temp_reshaped = temp_transposed.reshape(
                        -1, temp_transposed.shape[2] * temp_transposed.shape[3]
                    )
                    Hj_reshaped = Hj_transposed.reshape(
                        Hj_transposed.shape[0] * Hj_transposed.shape[1],
                        -1,
                    )
                    result = temp_reshaped @ Hj_reshaped
                    temp = result.reshape(
                        temp.shape[1], temp.shape[3], H[j].shape[0], H[j].shape[3]
                    )

                    # cap = np.einsum("ijkl,mlj->ikm",temp, psi[j])
                    temp_reshaped = temp.transpose(0, 2, 1, 3)
                    temp_reshaped = temp_reshaped.reshape(
                        -1, temp_reshaped.shape[2] * temp_reshaped.shape[3]
                    )
                    psi_j_reshaped = psi[j].transpose(2, 1, 0)
                    psi_j_reshaped = psi_j_reshaped.reshape(
                        psi_j_reshaped.shape[0] * psi_j_reshaped.shape[1], -1
                    )
...
def random_contraction_inc(
    ...
    for j in reversed(range(1, n)):
        ...
        if j == n - 1:
            ...
            prod_bond_dims = max(
                H[j].shape[0] * psi[j].shape[0], H[j].shape[2] * psi[j].shape[2]
            )  # either left or right
        ...
        while True:
            ...
            if j == psi.N - 1:
                ...
                prod = temp.reshape(temp.shape[0], -1) @ H_transposed.reshape(
                    -1, H_transposed.shape[-1]
                )  # Resulting shape Z,d
                temp = prod.T
            else:
                # temp = np.einsum('ZDX,XdY->ZDdY',all_env,psi[j])
                prod = all_env.reshape(-1, all_env.shape[-1]) @ psi[j].reshape(
                    psi[j].shape[0], -1
                )  # resulting shape(ZD,dY)
                temp = prod.reshape(
                    all_env.shape[0], all_env.shape[1], psi[j].shape[1], psi[j].shape[2]
                )  # reshape to (Z,D,d,Y)
                ...
                prod = temp_transposed.reshape(
                    -1, temp_transposed.shape[2] * temp_transposed.shape[3]
                ) @ H_transposed.reshape(
                    H_transposed.shape[0] * H_transposed.shape[1], -1
                )  # Resulting shape (ZY,dE)
                temp = prod.reshape(
                    temp.shape[0], temp.shape[3], H[j].shape[1], H[j].shape[2]
                )  # Reshape to ZYdE
                ...
                prod = temp.reshape(
                    -1, temp.shape[2] * temp.shape[3]
                ) @ cap_transposed.reshape(
                    -1, cap_transposed.shape[-1]
                )  # Resulting shape Zdp
                temp = prod.reshape(
                    temp.shape[0], temp.shape[1], cap.shape[0]
                )  # Reshape to Z,d,p
            ...
            if j == n - 1:
                ...
                if errorcount == 0:
                    temp = sketch[:, :, sketches_complete:current_sketchdim].reshape(
                        cap_dim * visible_dim, -1
                    )
            ...
            if outputdim is not None:  # If fixed dimension cutoff
                ...
                if current_sketchdim == current_maxdim:
                    ...
                    errorcount += 1
                    norm_est = np.linalg.norm(sketch.reshape(-1)) / np.sqrt(
                        current_sketchdim
                    )
                    done = (
                        err_est <= cutoff * norm_est
            ...
            if done:
                ...
                if j == n - 1:
                    ...
                    Hj_reshaped = Hj_transposed.reshape(
                        Hj_transposed.shape[0] * Hj_transposed.shape[1], -1
                    )
                    result = temp_reshaped @ Hj_reshaped
                    temp = result.reshape(
                        temp.shape[1], temp.shape[3], H[j].shape[0], H[j].shape[3]
...
def density_matrix(mpo, mps, stop=Cutoff(1e-14), maxdim=None, normalize=False):
    ...
    mpo_reshaped = mpo_c[0].reshape(
        mpo_c[0].shape[0] * mpo_c[0].shape[1], -1
    )  # Reshape to (dF, l)
    mps_reshaped = mps_c[0].reshape(-1, mps_c[0].shape[1])  # Reshape to (l, Z)
    temp = mpo_reshaped @ mps_reshaped
    temp = temp.reshape(
        mpo_c[0].shape[0], mpo_c[0].shape[1], mps_c[0].shape[1]
    )  # Reshape to (d, F, Z)
    ...
    temp_reshaped = temp_transposed.reshape(
        temp_transposed.shape[0], -1
    )  # Reshape to (d, FZ)
    L[0] = L_reshaped @ temp_reshaped  # Resulting shape (XD, FZ)
    L[0] = L[0].reshape(
        L_transposed.shape[0],
        L_transposed.shape[1],
        temp_transposed.shape[1],
        temp_transposed.shape[2],
    )  # Reshape to (X, D, F, Z)
    ...
    for j in range(1, n - 1):
        ...
        intermediate_reshaped = intermediate.reshape(
            L[j - 1].shape[0],
            L[j - 1].shape[1],
            L[j - 1].shape[2],
            mps_c[j].shape[1],
            mps_c[j].shape[2],
        )  # Reshape to (X, D, F, l, W)
        ...
        L_reshaped = L_transposed.reshape(
            L_transposed.shape[0] * L_transposed.shape[2] * L_transposed.shape[3], -1
        )  # Reshape to (XDW, Fl)
        mpo_reshaped = mpo_transposed.reshape(
            -1, mpo_transposed.shape[2] * mpo_transposed.shape[3]
        )  # Reshape to (Fl, dG)
        L[j] = L_reshaped @ mpo_reshaped  # Resulting shape (XDW, dG)
        L[j] = L[j].reshape(
            L_transposed.shape[0],
            L_transposed.shape[1],
            L_transposed.shape[2],
            mpo_transposed.shape[2],
            mpo_transposed.shape[3],
        )  # Reshape to (X, D, W, d, G)
        ...
        L_reshaped = L_transposed.reshape(
            L_transposed.shape[0] * L_transposed.shape[1] * L_transposed.shape[2], -1
        )  # Reshape to (XGW, Dd)
        mpo_reshaped = mpo[j].reshape(
            mpo[j].shape[0] * mpo[j].shape[1], mpo[j].shape[2] * mpo[j].shape[3]
        )  # Reshape to (Dd, El)
        ...
        L_reshaped = L_transposed.reshape(
            -1, L_transposed.shape[3] * L_transposed.shape[4]
        )  # Reshape to (EGW,Xl)
        mps_reshaped = mps[j].reshape(
            mps[j].shape[0] * mps[j].shape[1], mps[j].shape[2]
        )  # Reshape to (Xl,Y)
    ...
    intermediate = L_reshaped @ mps_reshaped
    rho = intermediate.reshape(
        L[n - 2].shape[0], L[n - 2].shape[1], L[n - 2].shape[2], mps_c[n - 1].shape[1]
    )  # Reshape to (X, D, F, l)

    # rho = np.einsum('XDFl,Fdl->XDd', rho, mpo_c[n-1])
    rho_reshaped = rho.reshape(
        rho.shape[0] * rho.shape[1], rho.shape[2] * rho.shape[3]
    )  # Reshape to (XD, Fl)
    mpo_transposed = mpo_c[n - 1].transpose(0, 2, 1)  # Transpose to (F, l, d)
    mpo_reshaped = mpo_transposed.reshape(
        mpo_transposed.shape[0] * mpo_transposed.shape[1], -1
    )  # Reshape to (Fl, d)
    ...
    rho = intermediate.T.reshape(
        mps[n - 1].shape[1], rho.shape[1], rho.shape[2]
    )  # Reshape to (l, D, d)

    # rho = np.einsum('lDd,Dol->od', rho, mpo[n-1])
    rho_transposed = rho.transpose(2, 1, 0)  # Transpose to (d, D, l)
    rho_reshaped = rho_transposed.reshape(
        -1, rho_transposed.shape[1] * rho_transposed.shape[2]
    )  # Reshape to (d, Dl)
    ...
    mpo_reshaped = mpo_transposed.reshape(
        -1, mpo_transposed.shape[1] * mpo_transposed.shape[2]
    )  # Reshape to (d, GL)
    ...
    M_top_reshaped = M_top.reshape(
        M_top.shape[0] * M_top.shape[1], -1
    )  # Reshape to (Gk, l)
    ...
    for j in reversed(range(1, n - 1)):
        ...
        M_top_reshaped = M_top_transposed.reshape(
            -1, M_top_transposed.shape[2]
        )  # Reshape to (Gk, W)
        ...
        top = top.reshape(
            mps_transposed.shape[1],
            mps_transposed.shape[2],
            M_top_transposed.shape[0],
            M_top_transposed.shape[1],
        )  # Reshape to (Z, l, G, k)

        # top = np.einsum("ZlGk,FdGl->ZFdk", top, mpo_c[j])
        top_transposed = top.transpose(0, 3, 2, 1)  # Transpose to (Z, k, G, l)
        top_reshaped = top_transposed.reshape(
            top_transposed.shape[0] * top_transposed.shape[1], -1
        )  # Reshape to (Zk, Gl)
        ...
        top = top.reshape(
            top_transposed.shape[0],
            top_transposed.shape[1],
            mpo_transposed.shape[2],
            mpo_transposed.shape[3],
        )  # Reshape to (Z, k, F, d)
        ...
        L_reshaped = L_transposed.reshape(
            L_transposed.shape[0] * L_transposed.shape[1], -1
        )  # Reshape to (FZ,XD)
        bottom_reshaped = bottom.reshape(
            bottom.shape[0] * bottom.shape[1], -1
        )  # Reshape to (XD ,dk)
        ...
        rho_reshaped = rho.reshape(
            -1, rho.shape[2] * rho.shape[3]
        )  # Reshape to (kd, FZ)
        top_transposed = top.transpose(1, 0, 2, 3)  # Transpose to (F, Z, e, j)
        top_reshaped = top_transposed.reshape(
            -1, top_transposed.shape[2] * top_transposed.shape[3]
        )  # Reshape to (FZ, ej)
        temp = rho_reshaped @ top_reshaped  # Resulting shape (kd, ej)
        rho = temp.reshape(
            rho.shape[0], rho.shape[1], top.shape[2], top.shape[3]
        )  # Reshape to (k, d, e, j)
        ...
        top_reshaped = top.reshape(
            top.shape[0], top.shape[1], -1
        )  # Reshape to (ZF, dk)
        U_reshaped = U.reshape(U.shape[0], -1)  # Reshape to (l, dk)
        intermediate = top_reshaped @ U_reshaped.T
        M_top = intermediate.reshape(
            top.shape[0], top.shape[1], U.shape[0]
        )  # Reshape to (Z, F, l)
    ...
    mps_out[0] = intermediate.reshape(
        mps_c[0].shape[0], M_top.shape[1], M_top.shape[2]
    )  # Reshape to (l, F, k)

    # mps_out[0] = np.einsum('lFk,dFl->dk', mps_out[0], mpo_c[0])
    mps_out_transposed = mps_out[0].transpose(2, 1, 0)  # Transpose to (k, F, l)
    mps_out_reshaped = mps_out_transposed.reshape(
        -1, mps_out_transposed.shape[1] * mps_out_transposed.shape[2]
    )  # Reshape to (k, Fl)
    mpo_transposed = mpo_c[0].transpose(1, 2, 0)  # Transpose to (F, l, d)
    mpo_reshaped = mpo_transposed.reshape(
        -1, mpo_transposed.shape[2]
    )  # Reshape to (Fl, d)
...
def zipup(mpo, mps, stop=Cutoff(1e-14), finalround=False, conditioning=False):
    ...
    mps_out[0], S, Vt = truncated_svd(
        np.reshape(C, (C.shape[0], C.shape[1] * C.shape[2])), stop=stop
    )
    Z = np.diag(S) @ Vt
    ...
    for i in range(1, n - 2):
        ...
        C_reshaped = C_transposed.reshape(
            -1, C_transposed.shape[2] * C_transposed.shape[3]
        )  # Reshape to (pY, Dl)
        mpo_reshaped = mpo_transposed.reshape(
            mpo_transposed.shape[0] * mpo_transposed.shape[1], -1
        )  # Reshape to (Dl, dE)
        ...
        temp = temp.reshape(
            C.shape[0], C.shape[3], mpo[i].shape[1], mpo[i].shape[2]
        )  # Reshape to (p, Y, d, E)
        C = temp.transpose(0, 2, 3, 1)  # Transpose to (p, d, E, Y)

        U, S, Vt = truncated_svd(
            C.reshape(C.shape[0] * C.shape[1], C.shape[2] * C.shape[3]), stop=stop
        )
    ...
    C = temp.reshape(
        Z.shape[0], Z.shape[1], mps[n - 2].shape[1], mps[n - 2].shape[2]
    )  # Reshape to (p, D, l, Y)
    ...
    temp = C_reshaped @ mpo_reshaped
    C = temp.reshape(
        C.shape[0], C.shape[1], mpo[n - 1].shape[1]
    )  # Reshape to (p, d, u)

    # print(C.shape)
    U, S, Vt = truncated_svd(
        np.reshape(C, (C.shape[0] * C.shape[1], C.shape[1])), stop=stop
    )
...
def zipup_randomsvd(
    ...
    # =============================== First tensor ===============================
    C = np.tensordot(mpo[0], mps[0], axes=(2, 0))
    mps_out[0], S, Vt = random_truncated_svd(
        np.reshape(C, (C.shape[0], C.shape[1] * C.shape[2])),
        num_iters,
        oversample,
        stop=stop,
    )
    ...
    # print(C.shape)
    U, S, Vt = random_truncated_svd(
        np.reshape(C, (C.shape[0] * C.shape[1], C.shape[1])),
        num_iters,
        oversample,
        stop=stop,
    )
...
def fit(
    ...
    def right_sweep(mps, mpo, L, final_site=None, stop=Cutoff(1e-14)):
        ...
        if final_site is None:
            ...
            site = temp.reshape(
                mps[-2].shape[0], mps[-2].shape[1], temp.shape[-1]
            )  # Reshape to (X,j,l)
            ...
            site = temp.reshape(
                site.shape[0], site.shape[1], mpo[-1].shape[0], mpo[-1].shape[1]
            )

            # site = np.einsum("XjEk,DdEj->XDkd", site, mpo[-2])
            site_transposed = site.transpose(0, 3, 1, 2)  # Transpose to (X,k,j,E)
            site_reshaped = site_transposed.reshape(
                -1, site_transposed.shape[2] * site_transposed.shape[3]
            )  # reshape to (Xk,jE)
            ...
            temp = temp.reshape(
                site.shape[0], site.shape[3], mpo[-2].shape0, mpo[-2].shape[1]
            )  # Reshape to (X,k,D,d)
            ...
            L_reshaped = L_transposed.reshape(
                L_transposed.shape[0], -1
            )  # Reshape to (p,XD)
            site_reshaped = site.reshape(
                -1, site.shape[2] * site.shape[3]
            )  # Reshape to (XD,kd)
            temp = L_reshaped @ site_reshaped  # resulting shape p,kd
            temp = temp.reshape(
                temp.shape[0], site.shape[2], site.shape[3]
            )  # Reshape to (p,k,d)
        ...
        U, S, Vt = truncated_svd(
            site.reshape(site.shape[0] * site.shape[1], site.shape[2]), stop=stop
        )
        ...
        temp = temp.reshape(
            mpo[-1].shape[0], mps[-1].shape[1], temp.shape[1]
        )  # Reshape to (D,d,X)
        ...
        R[-1] = temp.reshape(
            R[-1].shape[0], R[-1].shape[1], temp.shape[-1]
        )  # Reshape to (X,D,p)
        ...
        for k in range(mps.N - 2, 1, -1):
            ...
            R_reshaped = R_transposed.reshape(
                -1, R_transposed.shape[-1]
            )  # Reshape to (Dp,X)
            ...
            temp = temp.reshape(
                R[k - 1].shape[1], R[k - 1].shape[2], mps[k].shape[0], mps[k].shape[1]
            )  # Reshape to (D,p,Y,l)
            ...
            temp = temp.reshape(
                site.shape[0], site.shape[3], mpo[k].shape[0], mpo[k].shape[1]
            )  # reshape to (Y,p,E,d)
            ...
            site_reshaped = site_transposed.reshape(
                -1, site_transposed.shape[3] * site_transposed.shape[4]
            )  # Reshape to (Zdp,lE)
            ...
            L_reshaped = L_transposed.reshape(
                -1, L_transposed.shape[2]
            )  # Reshape to (FK,q)
            temp = site_reshaped @ L_reshaped  # Resulting shape (kdp,q)
            temp = temp.reshape(
                site.shape[2], site.shape[3], site.shape[4], L[k - 2].shape[0]
            )  # (k,d,p,q)
            site = temp.transpose(3, 0, 1, 2)  # Transpose to (q,k,d,p)

            U, S, Vt = truncated_svd(
                site.reshape(
                    site.shape[0] * site.shape[1], site.shape[2] * site.shape[3]
                ),
                stop=stop,
            )
            ...
            R_reshaped = R_transposed.reshape(
                -1, R_transposed.shape[2] * R_transposed.shape[3]
            )  # Reshape to (Xq,Dd)
            ...
            temp = temp.reshape(
                R[k - 2].shape[0], R[k - 2].shape[3], mpo[k].shape[0], mpo[k].shape[3]
            )  # Reshape tp (X,q,E,l)
            ...
            mpo_reshaped = mps_transposed.reshape(
                -1, mps_transposed.shape[-1]
            )  # reshape to (Xl,Y)
            temp = R_reshaped @ mpo_reshaped  # Resulting shape (Eq,Y)
            temp = temp.reshape(
                R[k - 2].shape[2], R[k - 2].shape[3], mps[k].shape[0]
            )  # Reshape to (E,q,Y)
        ...
        site = temp.reshape(
            mps[0].shape[0], mps[1].shape[1], mps[1].shape[2]
        )  # Reshape to (l,k,Y)
        ...
        mpo_reshaped = mpo_transposed.reshape(
            mpo_transposed.shape[0], -1
        )  # Reshape to (l,dD)
        temp = site_reshaped @ mpo_reshaped  # resulting shape (kY,dD)
        temp = temp.reshape(
            site.shape[1], site.shape[2], mpo[0].shape[0], mpo[0].shape[1]
        )  # Reshape to (k,Y,d,D)
        ...
        site_reshaped = site_transposed.reshape(
            site_transposed.shape[0] * site_transposed.shape[1], -1
        )  # Reshape to (kY,Dl)
        mpo_reshaped = mpo_transposed.reshape(
            mpo_transposed.shape[0] * mpo_transposed.shape[1], -1
        )  # Reshape tp (Dl,dE)
        ...
        final_site = temp.reshape(
            site.shape[0], site.shape[1], temp.shape[-1]
        )  # Reshape tp (k,d,q)

        U, S, Vt = truncated_svd(
            final_site.reshape(
                final_site.shape[0], final_site.shape[1] * final_site.shape[2]
            ),
            stop=stop,
        )
    ...
    def left_sweep(mps, mpo, R, final_site=None, stop=Cutoff(1e-14)):
        ...
        if final_site is None:
            ...
            site = temp.reshape(
                temp.shape[0], mps[1].shape[1], mps[1].shape[2]
            )  # Reshape to l,j,Y
            ...
            R_reshaped = R[0].reshape(
                R[0].shape[0] * R[0].shape[1], -1
            )  # Reshape to (YE,W)
        ...
        L[0] = temp.reshape(
            L[0].shape[0], L[0].shape[1], mps[0].shape[1]
        )  # Reshape to (p,D,X)
        ...
        for k in range(1, len(mps) - 2):
            ...
            site = temp.reshape(
                L[k - 1].shape[0], L[k - 1].shape[1], mps[k].shape[1], mps[k].shape[2]
            )  # Reshape to (p,D,l,Y)
            ...
            temp = temp.reshape(
                site.shape[0], site.shape[3], mpo[k].shape[1], mpo[k].shape[2]
            )  # Reshape to ( p,Y,d,E)
            ...
            site = temp.reshape(
                site.shape[0], site.shape[1], site.shape[2], R[k].shape[-1]
            )  # Reshape to (p,d,k,W)
            ...
            L_reshaped = L_transposed.reshape(
                -1, L_transposed.shape[-1]
            )  # Reshape to (Dx,p)
            mps_reshaped = mps_out[k].reshape(
                mps_out[k].shape[0], -1
            )  # Reshape to (p,dq)
            ...
            L_reshaped = L_transposed.reshape(
                -1, L_transposed.shape[2] * L_transposed.shape[3]
            )  # Reshape to (qX,Dd)
            mpo_reshaped = mpo[k].reshape(
                -1, mpo[k].shape[2] * mpo[k].shape[3]
            )  # Reshape to (Dd,El)
            temp = L_reshaped @ mpo_reshaped  # resulting shape (qX,El)
            temp = temp.reshape(
                L[k].shape[0], L[k].shape[3], mpo[k].shape[2], mpo[k].shape[3]
            )  # Reshape to (q,X,E,l)
            L[k] = temp.transpose(0, 2, 3, 1)  # Transpose to (q,E,l,X)

            # L[k] = np.einsum("qElX,XlY->qEY", L[k], mps[k])
            L_reshaped = L[k].reshape(
                -1, L[k].shape[2] * L[k].shape[3]
            )  # Reshape to (qE,lX)
        ...
        site = temp.reshape(
            mps[-2].shape[0], mps[-2].shape[1], mps[1].shape[1]
        )  # Reshape to (X,l,k)
        ...
        temp = temp.reshape(
            site.shape[0], site.shape[3], mpo[-2].shape[0], mpo[-2].shape[3]
        )  # Reshape to (X,d,D,k)
        ...
        mps_out[-2] = U.reshape(
            final_site.shape[0], mps[-2].shape[1], final_site.shape[2]
        )
        mps_out[-1] = (np.diag(S) @ Vt).reshape(final_site.shape[2], mps[-1].shape[1])

        return L, final_site, MPS(mps_out)

    def compute_left_envs(
        mps, mpo, guess
    ):  # TODO: Convert to einsums if needed ie starting from right
        L = [None] * (mps.N - 2)
        # -------- Left environments --------
    ...
    def compute_right_envs(mps, mpo, guess):
        ...
        mpo_reshaped = mpo_transposed.reshape(
            -1, mpo_transposed.shape[-1]
        )  # Reshape to (Dl,d)
        temp = mpo_reshaped @ guess[-1].T  # Resulting shape to (Dl,Z)
        temp = temp.reshape(
            mpo[-1].shape[0], mpo[-1].shape[1], temp.shape[-1]
        )  # Reshape to (D,l,Z)
        ...
        temp = temp.reshape(
            R[-1].shape[0], R[-1].shape[1], mps[-1].shape[0]
        )  # Reshape to (Z,D,X)
        ...
        for i in range(mps.N - 2, 1, -1):
            ...
            temp = temp.reshape(
                R[i - 1].shape[1], R[i - 1].shape[2], mps[i].shape[0], mps[i].shape[1]
            )  # Reshape to (D,Z,Y,l)
            ...
            temp = temp.reshape(
                R[i - 2].shape[0], R[i - 2].shape[3], mpo[i].shape[0], mpo[i].shape[1]
            )  # reshape to (Y,Z,E,d)
            R[i - 2] = temp.transpose(0, 2, 3, 1)

            # R[i - 2] = np.einsum("YEdZ,WdZ->YEW", R[i - 2], guess[i])
            R_reshaped = R[i - 2].reshape(
                -1, R[i - 2].shape[2] * R[i - 2].shape[3]
            )  # reshape to (YE,dZ)
            guess_transposed = guess[i].transpose(1, 2, 0)  # Transpose to (d,Z,W)
            guess_reshaped = guess_transposed.reshape(
                -1, guess_transposed.shape[-1]
            )  # Reshape to (dZ,W)
            temp = R_reshaped @ guess_reshaped  # Resulting shape (YE,W)
            R[i - 2] = temp.reshape(
                R[i - 2].shape[0], R[i - 2].shape[1], guess[i].shape[0]
            )  # Reshape to (Y,E,W)
        return R
    ...
    if guess is None:
        guess = MPS.random_mps(
            n=mps.N, m=mps[0].shape[1], d=mps[0].shape[0], random_tensor=random_tensor
        )
    elif guess == "input":
        guess = mps.copy()
    ...
    for sweep_count in range(max_sweeps):
        L, final_site, mps_approx = left_sweep(
            mps, mpo, R, stop=stop, final_site=final_site
        )
        R, final_site, mps_approx = right_sweep(
            mps, mpo, L, stop=stop, final_site=final_site
        )
...
def mps_mpo(
    ...
    new_mps.append(np.reshape(site, (site.shape[0], site.shape[1] * site.shape[2])))
...
def fit_einsum(
    mpo, mps, max_sweeps=10, stop=Cutoff(1e-14), random_tensor=np.random.randn
):
    """
    Einsum Indexing convention L->R:
     ______                 ______
...
def rand_apply_torch_tensordot(
    ...
    for j in reversed(range(1, n)):
        ...
        if j == n - 1:
            sketch = torch.zeros(
                (visible_dim, current_sketchdim), dtype=torch.complex128
            )
        else:
            sketch = torch.zeros(
                (visible_dim, cap_dim, current_sketchdim), dtype=torch.complex128
            )
