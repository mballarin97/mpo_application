...
class MPO:
    ...
    def from_list(cls, tensors):
        new_tensors = [np.squeeze(tensors[0].tt).transpose(0, 2, 1)]
        new_tensors += [ tt.tt.transpose(0, 1, 3, 2) for tt in tensors[1:-1]]
        new_tensors += [ np.squeeze(tensors[-1].tt) ]
    ...
    def canonize_right(self):
        ...
        for i in range(1, self.N - 1):
            ...
            Q, R = la.qr(
                np.reshape(A, (A.shape[0] * A.shape[1] * A.shape[2], A.shape[3]))
            )
    ...
    def norm(self):
        ...
        if "Right" == self.canform:
            reshaped_tensor = np.reshape(
                self[-1], (self[-1].shape[0], self[-1].shape[1] * self[-1].shape[2])
            )
            return np.linalg.norm(reshaped_tensor, "fro")
        else:
            reshaped_tensor = np.reshape(
                self[0], (self[0].shape[0] * self[0].shape[2], self[0].shape[1])
            )
            return np.linalg.norm(reshaped_tensor, "fro")
    ...
    def __mul__(self, down):
        ...
        site = np.reshape(
            site, (site.shape[0], site.shape[1] * site.shape[2], site.shape[3])
        )
        mpo.append(site)
        ...
        for i in range(1, len(self) - 1):
            site = np.tensordot(
                self[i], down[i], (3, 1)
            )  # site(self0,self1,self2,down0,down2,down3)
            site = np.transpose(
                site, (0, 3, 1, 2, 4, 5)
            )  # site(self0,down0,self1,self2,down2,down3)
            site = np.reshape(
                site,
                (
    ...
    def add(self, mpo2, subtract=False, compress=False, stop=Cutoff(1e-14)):
        ...
        m1 = self[0].shape[1]
        m2 = mpo2[0].shape[1]
        new_left_boundary = np.zeros(
            (self[0].shape[0], m1 + m2, self[0].shape[2]), dtype="complex"
        )
        ...
        for i in range(1, self.N - 1):
            ...
            n2 = mpo2[i].shape[2]
            new_core = np.zeros(
                (m1 + m2, self[i].shape[1], n1 + n2, self[i].shape[3]), dtype="complex"
            )
        ...
        m1 = self[-1].shape[0]
        m2 = mpo2[-1].shape[0]
        new_right_boundary = np.zeros(
            (m1 + m2, self[-1].shape[1], self[-1].shape[2]), dtype="complex"
        )
    ...
    def round(self, stop=Cutoff(1e-14)):
        ...
        U, S, Vt = truncated_svd(
            self[0].reshape(self[0].shape[0] * self[0].shape[2], -1), stop=stop
        )
        ...
        for i in range(1, self.N - 1):
            ...
            U, S, Vt = truncated_svd(
                np.reshape(A, (A.shape[0] * A.shape[1] * A.shape[3], A.shape[2])),
                stop=stop,
            )
    ...
    def is_right_orth(self):
        ...
        for i in reversed(range(1, self.N)):
            ...
            if i == self.N - 1:
                ...
                reshaped_tensor = tensor.reshape(
                    tensor.shape[0], tensor.shape[1] * tensor.shape[3] * tensor.shape[2]
                )
    ...
    def random_mpo(n, m, d=2, d2=2, dtype=float, random_tensor=np.random.randn):
        ...
        return MPO(
            [random_tensor_with_dtype(d, m, d2)]
    ...
    def random_incremental_mpo(N, d, seed, d2=None):
        if d2 is None:
            d2 = d
        mpo = [
            np.random.randn(d, seed, d2)
        ]  # Initialize with the first tensor, bond dimension 1 for the first tensor
