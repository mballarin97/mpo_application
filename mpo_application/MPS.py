...
class MPS:
    ...
    def from_list(cls, tensors):
        new_tensors = [tt.tt for tt in tensors]
        new_tensors[0] = new_tensors[0].reshape(new_tensors[0].shape[1:])
        new_tensors[-1] = new_tensors[-1].reshape(new_tensors[-1].shape[:-1])
    ...
    def canonize_left(self):
        ...
        for i in range(self.N - 2, 0, -1):
            ...
            Q = Qt.T
            self[i] = np.reshape(
                Q, (min(A.shape[0], A.shape[1] * A.shape[2]), A.shape[1], A.shape[2])
            )
            L = R.T
        self[0] = self[0] @ L
        self.canform = "Left"
    ...
    def canonize_right(self):
        ...
        for i in range(1, self.N - 1):
            ...
            self[i] = np.reshape(
                Q, (A.shape[0], A.shape[1], min(A.shape[2], A.shape[0] * A.shape[1]))
            )
    ...
    def round(
        self, **kwargs
    ):  # TODO add support for other rounding in randomcontraction.rounding
        from .rounding import (
            round_left,
            round_right,
        )  # TODO:possibly slow way of handling circular import exception
    ...
    def contract_all(self):
        ...
        for i in range(1, self.N):
            ...
            if i == 1:
                result = np.tensordot(
                    result, self[i], axes=([1], [0])
                )  # contract over the last dimension of result and the first dimension of mps[i]
            else:
    ...
    def random_mps(n, m, d=2, random_tensor=np.random.randn, dtype=float):
        return MPS(
            [random_tensor(d, m).astype(dtype)]
            + [random_tensor(m, d, m).astype(dtype) for _ in range(n - 2)]
            + [random_tensor(m, d).astype(dtype)]
        )

    @staticmethod
    def random_incremental_mps(n, d, seed, dtype=np.complex128):
        mps = [
            np.random.randn(d, seed).astype(dtype)
